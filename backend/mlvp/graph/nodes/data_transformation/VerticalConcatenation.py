...
CONCATENATE_H = "{df} = pd.concat([{x},{y}], join = 'outer', axis = 1)\n"
CONCATENATE_V = "{df} = pd.concat([{top_df},{bot_df}])\n"
X = "{x} = {df}.drop({df}.columns[-1], axis=1)\n"
Y = "{y} = {df}[{df}.columns[-1]]\n"


class VerticalConcatenation(Node):
    ...
    def codegen(self, emitter: Emitter, out_file):
        ...
        x = "x" + str(curr_count)
        y = "y" + str(curr_count)
        out_file.write(CONCATENATE_H.format(df=top_df, x=top_x, y=top_y))
        out_file.write(CONCATENATE_H.format(df=bot_df, x=bot_x, y=bot_y))
        out_file.write(CONCATENATE_V.format(df=final_df, top_df=top_df, bot_df=bot_df))
