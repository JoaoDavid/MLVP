...
class Verification:
    ...
    def __convert_ids(self, expr: ExprRef):
        ...
        if expr.num_args() == 0:
            ...
            decl = str(expr.decl())
            if decl in ["==", "!=", "<", "<=", ">", ">=", "+", "-", "*", "/"]:
                return BINARY_OPERATOR.format(left=self.__convert_ids(children[0]), b_op=decl,
                                              right=self.__convert_ids(children[1]))
            elif decl == "Not":
                return UNARY_OPERATOR.format(u_op="~", expr=self.__convert_ids(children[0]))
            elif decl == "And":
                return BINARY_OPERATOR.format(left=self.__convert_ids(children[0]), b_op="&&",
                                              right=self.__convert_ids(children[1]))
            elif decl == "Or":
                return BINARY_OPERATOR.format(left=self.__convert_ids(children[0]), b_op="||",
                                              right=self.__convert_ids(children[1]))
